...
class WalletGroupsService {
  ...
  Future<List<WalletGroupModel>> getWalletGroups(String path, {bool strictBool = false}) async {
  ...
  Future<WalletGroupModel?> getGroupByPath(String path) async {
    try {
  ...
  Future<void> moveGroup(ContainerPathModel previousPath, ContainerPathModel newPath) async {
    WalletGroupModel? previousWalletGroupModel = await getGroupByPath(previousPath.fullPath);
  ...
  Future<void> deleteGroupByPath(String path, {bool recursiveBool = false}) async {
    if (recursiveBool) {
      ...
      List<WalletGroupModel> walletGroupModels = await getWalletGroups(path, strictBool: true);
      for (WalletGroupModel walletGroupModel in walletGroupModels) {
        await deleteGroupByPath(walletGroupModel.containerPathModel.fullPath, recursiveBool: recursiveBool);
      }
    }
