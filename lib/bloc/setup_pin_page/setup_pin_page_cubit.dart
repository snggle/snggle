...
class SetupPinPageCubit extends Cubit<ASetupPinPageState> {
  ...
  Future<void> _comparePin(String pin) async {
    if (setupPin == pin) {
      try {
        await _authService.storeAuthentication(pin: pin).whenComplete(() => emit(SetupPinPageSuccessState()));
      } catch (e) {
  ...
  Future<void> setupUpLater() async {
    try {
      await _authService.authenticateLater().whenComplete(() => emit(SetupPinPageLaterState()));
    } catch (e) {
      AppLogger().log(message: e.toString());
      emit(SetupPinPageFailState());
    }
