...
class WalletConnectPageCubit extends Cubit<WalletConnectPageState> {
  ...
  void changeExportType(WalletExportType walletExportType) {
    emit(WalletConnectPageState(walletExportType: walletExportType));
  }

  Future<CborCryptoHDKey> getCborCryptoHDKey({required bool exportAllBool}) async {
    ...
    List<LegacyDerivationPathElement> parentPathElements = legacyDerivationPath.pathElements.sublist(
      0,
      legacyDerivationPath.pathElements.length - (exportAllBool ? 2 : 1),
    );
