...
class VaultListItemCubit extends Cubit<VaultListItemState> {
  ...
  Future<void> init() async {
    ...
    if (vaultModel.passwordProtectedBool) {
  ...
  Future<void> setPassword(PasswordModel newPasswordModel) async {
    vaultModel.passwordProtectedBool = true;
    await _vaultsService.saveVault(vaultModel);
    await _secretsService.changeParentPassword(vaultModel.containerPathModel, PasswordModel.defaultPassword(), newPasswordModel);
    emit(state.copyEncrypted(lockedBool: false));
  }

  Future<void> removePassword(PasswordModel oldPasswordModel) async {
    vaultModel.passwordProtectedBool = false;
    await _vaultsService.saveVault(vaultModel);
    await _secretsService.changeParentPassword(vaultModel.containerPathModel, oldPasswordModel, PasswordModel.defaultPassword());
