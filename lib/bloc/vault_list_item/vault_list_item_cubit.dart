...
class VaultListItemCubit extends Cubit<VaultListItemState> {
  ...
  Future<void> init() async {
    bool defaultPasswordBool = await _secretsService.isSecretsPasswordValid(vaultModel.path, PasswordModel.defaultPassword());
  ...
  Future<void> setPassword(PasswordModel newPasswordModel) async {
    await _secretsService.changeParentPassword(vaultModel.path, PasswordModel.defaultPassword(), newPasswordModel);
    emit(state.copyEncrypted(lockedBool: false));
  }

  Future<void> removePassword(PasswordModel oldPasswordModel) async {
    await _secretsService.changeParentPassword(vaultModel.path, oldPasswordModel, PasswordModel.defaultPassword());
  ...
  Future<void> unlock(PasswordModel passwordModel) async {
    bool passwordValid = await _secretsService.isSecretsPasswordValid(vaultModel.path, passwordModel);
  ...
  Future<VaultSecretsModel> getVaultSecrets(PasswordModel passwordModel) async {
    VaultSecretsModel vaultSecretsModel = await _secretsService.getSecrets(vaultModel.path, passwordModel);
