...
class SecretsSetupPinPageCubit<T extends ASecretsModel> extends Cubit<ASecretsSetupPinPageState> {
  ...
  Future<void> _savePin(PasswordModel pinPasswordModel) async {
    ...
    for (AContainerModel containerModel in containerModels) {
      try {
        T secretsModel = await _secretsService.getSecrets(containerModel.containerPathModel, PasswordModel.defaultPassword());
        await _secretsService.saveSecrets(secretsModel, pinPasswordModel);
      } catch(e) {
