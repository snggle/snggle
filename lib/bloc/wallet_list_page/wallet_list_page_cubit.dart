...
class WalletListPageCubit extends AListCubit<AListItem> {
  ...
  @override
  Future<void> deleteFromDatabase(AListItem item) async {
    if (item is WalletListItemModel) {
      ...
      await _walletGroupsService.deleteGroupByPath(item.walletGroupModel.containerPathModel.fullPath, recursiveBool: true);
    } else {
  ...
  @override
  Future<List<AListItem>> fetchAllFromDatabase() async {
    List<WalletModel> walletModelList = await _walletsService.getWalletList(containerPathModel.fullPath, strictBool: true);
    List<WalletGroupModel> walletGroupModelList = await _walletGroupsService.getWalletGroups(containerPathModel.fullPath, strictBool: true);
  ...
  @override
  Future<void> saveToDatabase(AListItem item) async {
    if (item is WalletListItemModel) {
  ...
  @override
  Future<List<AListItem>> filterBySearchPattern(List<AListItem> allItems, String searchPattern) async {
    ...
    return <AListItem>[...filteredGroups, ...filteredWallets];
  }

  @override
  List<AListItem> sortItems(List<AListItem> items) {
    ...
    return <AListItem>[...pinnedWalletGroups, ...unpinnedWalletGroups, ...pinnedWallets, ...unpinnedWallets];
  }

  Future<void> groupSelection({required List<AListItem> selectedItems}) async {
    ...
    for (AListItem item in selectedItems) {
      if (item is WalletListItemModel) {
