...
Future<void> main() async {
  ...
  late SecretsRemovePinPagePageCubit actualSecretsRemovePinPagePageCubit;

  group('Tests of [SecretsRemovePinPagePageCubit]', () {
    group('Tests of [SecretsRemovePinPagePageCubit] when [pin CORRECT]', () {
      setUpAll(() {
        ...
        actualSecretsRemovePinPagePageCubit = SecretsRemovePinPagePageCubit(
          containerModel: actualWalletGroupModel,
      ...
      test('Should [emit SecretsAuthPageEnterPinState] with [EMPTY pinNumbers] as initial state', () async {
        ...
        expect(actualSecretsRemovePinPagePageCubit.state, expectedSecretsAuthPageState);
      });

      test('Should [emit SecretsAuthPageEnterPinState] with [FILLED pinNumbers]', () async {
        // Act
        actualSecretsRemovePinPagePageCubit.updatePinNumbers(const <int>[1, 1, 1, 1]);
      ...
      test('Should [return PasswordEntryResultModel] if provided [password VALID]', () async {
        // Act
        PasswordEntryResultModel actualPasswordEntryResultModel = await actualSecretsRemovePinPagePageCubit.authenticate();

        // Assert
        PasswordEntryResultModel expectedPasswordEntryResultModel = PasswordEntryResultModel(
          passwordModel: PasswordModel.fromPlaintext('1111'),
          validBool: true,
        );

        expect(actualPasswordEntryResultModel, expectedPasswordEntryResultModel);
      });
    });

    group('Tests of [SecretsRemovePinPagePageCubit] when [pin INCORRECT]', () {
      setUpAll(() {
        testSessionUUID = const Uuid().v4();
        TestUtils.setupTmpFilesystemStructureFromJson(actualFilesystemStructure, path: testSessionUUID);
      ...
      test('Should [emit SecretsAuthPageEnterPinState] with [FILLED pinNumbers]', () async {
        // Act
        actualSecretsRemovePinPagePageCubit.updatePinNumbers(const <int>[9, 9, 9, 9]);
      ...
      test('Should [throw InvalidPasswordException] and [emit SecretsAuthPageInvalidPinState] if provided [password INVALID]', () async {
        try {
          // Act
          await actualSecretsRemovePinPagePageCubit.authenticate();
