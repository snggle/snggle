...
void main() {
  group('Tests of Bip39.generateMnemonic()', () {
    test('Should return mnemonic with 24 words', () {
      // Act
      MnemonicModel actualMnemonicModel = Bip39.generateMnemonic();
      int actualMnemonicLength = actualMnemonicModel.value.split(' ').length;

      // Assert
      int expectedMnemonicLength = 24;

      // Because the mnemonic is generated randomly, it is not possible to predict the result.
      // Therefore, the test checks only the length of generated mnemonic.
      expect(actualMnemonicLength, expectedMnemonicLength);
    });
  });

  group('Tests of Bip39.mnemonicToSeed()', () {
    test('Should return valid seed for mnemonic with 12 words', () async {
      ...
      // Act
      Uint8List actualSeed = await Bip39.mnemonicToSeed(mnemonic: actualMnemonicModel.value);
